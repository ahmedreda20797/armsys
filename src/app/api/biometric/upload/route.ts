...
export async function POST(request: NextRequest) {
  try {
    ...
    // The import writes biometric records for employees matched by
    // free-text name/code AND CREATES new (org-unassigned) employees
    // for unmatched names — its output cannot be confined to a
    // restricted employee scope. Like every workforce-create path it
    // therefore requires an UNRESTRICTED employee scope; scoped
    // viewers are denied fail-closed. (Biometric records are DATA
    // RECORDS of employees — the operator is never assumed to be the
    // target employee.)
    ...
    if (!unrestricted) {
      return NextResponse.json(
        { error: 'صلاحية غير كافية لاستيراد بيانات بصمة خارج نطاق بياناتك' },
        { status: 403 },
      );
    }
    ...
    for (let i = headerIdx + 1; i < rawData.length; i++) {
      ...
      if (matched) {
        ...
        if (existing) {
          ...
          const emp = await createRecord('employees', { name, code: code || null, status: DEFAULT_EMPLOYEE_STATUS });
          employeeId = emp.id;
