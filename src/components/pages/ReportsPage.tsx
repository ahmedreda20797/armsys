...
export default function ReportsPage() {
  ...
  const getComplianceBadgeBg = (val: number) => {
    if (val >= 90) return 'bg-emerald-500/15 border-emerald-500/25';
  ...
  const getDayStatusBadge = (status: string) => {
    switch (status) {
      case 'present': return <Badge className="bg-emerald-500/15 text-emerald-400 border-emerald-500/20 text-[11px]">حاضر</Badge>;
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs mt-0.5">تقرير الخصومات والحضور الشهري الشامل</p>
          </div>
        </div>
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          ...
          {summary && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
              ...
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.06 }} className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 px-3.5 py-3">
                <div className="flex items-center gap-1.5 mb-1.5"><CalendarCheck className="size-3.5 text-emerald-400" /><span className="text-slate-400 text-[10px] font-medium">إجمالي الحضور</span></div>
                <p className="text-2xl font-bold text-emerald-400 leading-tight" dir="ltr">{summary.totalPresentDays}</p>
            ...
            <Table dir="rtl">
              ...
              <TableBody>
                      ...
                      >
                        <TableCell className="py-3 px-3">
                          <div className="flex items-center gap-2.5">
                            <div className={`size-9 rounded-full flex items-center justify-center text-xs font-bold shrink-0 border ${row.attendanceCompliance >= 90 ? 'bg-emerald-500/15 border-emerald-500/25 text-emerald-400' : row.attendanceCompliance >= 75 ? 'bg-amber-500/15 border-amber-500/25 text-amber-400' : 'bg-red-500/15 border-red-500/25 text-red-400'}`}>
                              {row.employeeName.charAt(0)}
                            </div>
                            <div className="min-w-0 flex-1">
                              <p className="text-white font-medium text-sm">{row.employeeName}</p>
                        ...
                        <TableCell className="text-center py-3 px-3"><span className="text-emerald-400 font-bold text-sm">{row.totalPresent}</span></TableCell>
                                  ...
                                  <div className="p-6 space-y-6">
                                    ...
                                    <div className="rounded-xl border border-slate-700/40 bg-slate-800/60 p-5">
                                      ...
                                      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-3">
                                        ...
                                        <StatBox label="حضور" value={`${detailData.reportSummary.totalPresent}`} color="text-emerald-400" />
                                    ...
                                    <div className="rounded-xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 p-5">
                                      ...
                                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                        <div className="rounded-lg bg-slate-900/60 border border-slate-700/30 px-4 py-3 text-center">
                                          ...
                                          <p className="text-2xl font-bold text-emerald-400" dir="ltr">{detailData.reportSummary.totalPresent + detailData.reportSummary.totalLate}</p>
                                        ...
                                        <div className="rounded-lg bg-slate-900/60 border border-slate-700/30 px-4 py-3 text-center">
                                          ...
                                          <p className="text-2xl font-bold text-emerald-400" dir="ltr">{detailData.reportSummary.totalPresent}</p>
                                              ...
                                              <TableRow key={dIdx} className={`border-slate-700/20 hover:bg-slate-700/15 ${day.waived ? 'bg-emerald-500/[0.04]' : day.status === 'absent' ? 'bg-red-500/[0.03]' : day.status === 'exempt' ? 'bg-cyan-500/[0.03]' : ''}`}>
                                                ...
                                                <TableCell className="py-2 px-3">
                                                  <div className="flex items-center gap-1.5">
                                                    ...
                                                    {day.waived && !day.autoFree && <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-[10px]">ملغى يدوياً</Badge>}
                                                  </div>
                                                </TableCell>
                                                ...
                                                <TableCell className="text-center py-2 px-3 text-xs">
                                                  {day.absenceDeduction > 0 ? <span className="text-red-400 font-medium" dir="ltr">{day.absenceDeduction}</span> : day.waived && day.waivedType === 'absence' ? <span className="text-emerald-400 text-[10px]">0 (ملغى)</span> : day.autoFree ? <span className="text-blue-400 text-[10px]">0 (إعفاء تلقائي)</span> : <span className="text-slate-600">—</span>}
                                                </TableCell>
                                                <TableCell className="text-center py-2 px-3 text-xs">
                                                ...
                                                <TableCell className="text-center py-2 px-2">
                                                  ...
                                                  {canEdit && day.waived && (
                                                    <button
                                                      ...
                                                      disabled={waivingDate === day.date}
                                                      className="p-1 rounded-md bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 hover:text-emerald-300 transition-colors disabled:opacity-50"
                                                      title="استعادة الخصم"
                                                    >
                ...
                <TableRow className="border-t-2 border-slate-600/50 bg-slate-900/70 hover:bg-transparent font-bold">
                  ...
                  <TableCell />
                  <TableCell className="text-center py-3 px-3"><span className="text-emerald-400 text-sm font-bold">{totalPresent}</span></TableCell>
