...
export async function POST(request: NextRequest) {
  try {
    // M0.1: travel import performs bulk writes (deals + auto-created
    // employees) — require the travel 'create' action.
    ...
    // The import writes deals for SHEET-MATCHED employees and CREATES
    // new employee records — its output cannot be confined to a
    // restricted employee set, so a scoped viewer is denied fail-closed
    // (the canonical hasUnrestrictedEmployeeScope rule for bulk
    // uploads; permission alone never implies workforce-wide writes).
    ...
    if (!unrestricted) {
      return NextResponse.json(
        { error: 'استيراد السفر يتطلب صلاحية نطاق كامل على الموظفين' },
        { status: 403 },
      );
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        const empResult = await findOrCreateEmployee(deal.employeeName, allEmployees as EmpRecord[]);
